		...
		>
			<div class="flex items-center justify-between">
				...
				<button
					...
					on:click={() => (open = true)}
					class="block p-2 text-2xl text-slate-800 md:hidden"><IconMenu /></button
				>
			</div>
			<!-- Mobile Nav -->
			<ul
				class="fixed inset-0 z-50 flex flex-col items-end gap-4 bg-slate-50 pr-4 pt-14 transition-transform duration-300 ease-in-out md:hidden {open
					? 'translate-x-0'
					: 'translate-x-[100%]'}"
			>
				<li>
					<button
						...
						on:click={() => (open = false)}
						class="fixed right-4 top-3 block p2 text-2xl text-slate-800 md:hidden"
						><IconClose /></button
					>
				</li>
				{#each settings.data.nav_item as { label, link }}
					<li class="first:mt-8">
						<NavbarLink field={link} {label} {onLinkClick} type="mobile"></NavbarLink>
					</li>
				{/each}
			...
			<ul class="relative z-50 hidden flex-row items-center gap-1 bg-transparent py-0 md:flex">
				{#each settings.data.nav_item as { label, link }}
					<li>
						<NavbarLink field={link} {label} {onLinkClick} type="desktop"></NavbarLink>
					</li>
				{/each}
				{#if isFilled.link(settings.data.cta_link)}
