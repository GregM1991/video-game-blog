...
{#if type === 'desktop'}
	...
	>
		<span
			class="absolute inset-0 z-0 h-full rounded bg-yellow-300 transition-transform duration-300 ease-in-out group-hover:translate-y-0 {isActive
				? 'translate-y-6'
				: 'translate-y-8'}"
		></span>
		<span class="relative">
...
{:else}
	<PrismicLink
		class="group relative block overflow-hidden rounded text-3xl px-3 font-bold text-slate-900"
		{field}
		on:click={onLinkClick}
		aria-current={isActive ? 'page' : undefined}
	>
		<span
			class="absolute inset-0 z-0 h-full rounded bg-yellow-300 transition-transform duration-300 ease-out group-hover:translate-y-0 {isActive
				? 'translate-y-6'
				: 'translate-y-16'}"
		></span>
		<span class="relative">
